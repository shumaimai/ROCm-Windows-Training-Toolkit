...
$Command = "call `"$VcVars`" >nul && `"$Python`" -c `"from wavetrain.hip_extension import load_extension, extension_info; load_extension(); print(extension_info())`""
cmd.exe /d /s /c $Command
if ($LASTEXITCODE -ne 0) { throw "WaveTrain HIP extension build failed." }
