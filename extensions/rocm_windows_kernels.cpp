...
std::vector<torch::Tensor> rocm_windows_ssd_forward(
    torch::Tensor x,
    torch::Tensor dt,
    torch::Tensor a,
    torch::Tensor b,
  ...
  return rocm_windows_ssd_forward_hip(x, dt, a, b, c, d, z, dt_bias, seq_idx);
}
...
std::vector<torch::Tensor> rocm_windows_ssd_backward(
    torch::Tensor grad_output,
    torch::Tensor states,
    torch::Tensor x,
    torch::Tensor dt,
    ...
    torch::Tensor dt_bias,
    torch::Tensor seq_idx) {
  return rocm_windows_ssd_backward_hip(
...
torch::Tensor rocm_windows_conv_forward(
    torch::Tensor x, torch::Tensor weight, torch::Tensor seq_idx) {
  ...
  return rocm_windows_conv_forward_hip(x, weight.contiguous(), seq_idx.contiguous());
}

std::vector<torch::Tensor> rocm_windows_conv_backward(
    torch::Tensor grad_output, torch::Tensor x, torch::Tensor weight,
    torch::Tensor seq_idx, bool need_weight_grad) {
  return rocm_windows_conv_backward_hip(
      grad_output.contiguous(), x, weight.contiguous(), seq_idx.contiguous(),
      need_weight_grad);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("forward", &rocm_windows_ssd_forward, "SSD-style fused HIP forward");
  module.def("backward", &rocm_windows_ssd_backward, "SSD-style fused HIP backward");
  module.def("conv_forward", &rocm_windows_conv_forward, "Causal conv HIP forward");
  module.def("conv_backward", &rocm_windows_conv_backward, "Causal conv HIP backward");
}
